...
const CreateTopic = forwardRef((props, ref: ForwardedRef<any>) => {



    const { core, getCoreResources } = useApp()
    const { loading, fieldRef, skillRef, statusRef, topicData, field, skillList,
        setField, addSkill, removeSkill, handleChange, createTopic
    } = useTopic()
    const { goTo } = useGoTo()
    ...
    const validateSkill = () => {
        ...
        if (!topicData.name) {
            setToast({ ...toast, show: true, error: 'topic', type: 'error', message: 'Topic name is required' })
            setTimeout(() => {
                setToast({ ...toast, show: false })
            }, 2500)
            isValid = false;

        } else if (!topicData.label) {
            setToast({ ...toast, show: true, error: 'topic', type: 'error', message: 'Topic display name is required' })
            setTimeout(() => {
                setToast({ ...toast, show: false })
            }, 2500)
            isValid = false;
        }
        else if (!topicData.fieldId) {
            setToast({ ...toast, show: true, error: 'topic', type: 'error', message: 'Field is required' })
            setTimeout(() => {
                setToast({ ...toast, show: false })
            }, 2500)
            isValid = false;
        }
        else if (topicData.skills.length === 0) {
            setToast({ ...toast, show: true, error: 'topic', type: 'error', message: 'At least one skill is required' })
            setTimeout(() => {
                setToast({ ...toast, show: false })
            }, 2500)
            isValid = false;
        } else if (topicData.description.length < 10) {
            setToast({ ...toast, show: true, error: 'topic', type: 'error', message: 'Description must be at least 10 characters long' })
            setTimeout(() => {
                setToast({ ...toast, show: false })
            }, 2500)
            isValid = false;
        ...
        <>
                ...
                <form onSubmit={(e) => { e.preventDefault() }} className="w-[45%] mx-auto mt-5 mb-10">
                    ...
                    <div className="w-full space-y-[0.55rem]">
                        ...
                        <div className="flex items-center">
                            <h3 className="font-mona text-[13px]">Field</h3>
                        </div>
                        ...
                        <div className="w-full flex items-start gap-x-[1rem]">
                            ...
                            <div className="min-w-[48%]">
                                <Filter
                                    ...
                                    noFilter={false}
                                    onChange={(data) => {
                                        handleChange('fieldId', data.value)
                                        setField({ _id: data.value, name: data.label })
                                    }}
                                />
                            </div>
                        ...
                        <div className="w-full flex items-start gap-x-[1rem]">
                            ...
                            <div className="min-w-[48%]">
                                <Filter
                                    ...
                                    noFilter={false}
                                    onChange={(data) => {
                                        addSkill({ id: data.value, name: data.label, ex: false })
                                    }}
                                />
                            </div>
                        ...
                        <div className="w-full flex items-start gap-x-[1rem]">
                            ...
                            <div className="min-w-[40%]">
                                <Filter
                                    ...
                                    noFilter={false}
                                    defaultValue={topicData.isEnabled ? 'enable' : 'disable'}
                                    onChange={(data) => {
                            ...
                            <FormField className="grow flex flex-wrap items-center gap-x-[0.5rem] gap-y-[0.5rem]">
                                ...
                                <Badge
                                    type={topicData.isEnabled === true ? 'success' : 'error'}
                                    size="xsm"
                                    close={false}
                                    label={`${helper.capitalize(topicData.isEnabled ? 'Enabled' : 'Disabled')}`}
                                    upper={true}
                                />
            ...
            <div className="flex justify-end items-center gap-x-[0.65rem] mt-10">
                ...
                <Button
                    ...
                    loading={loading}
                    onClick={async (e) => { console.log(topicData); createTopic(validateSkill) }}
                />
