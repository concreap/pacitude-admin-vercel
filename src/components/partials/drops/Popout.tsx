...
const Popout = forwardRef((props: IPopout, ref: ForwardedRef<any>) => {
    ...
    const cc = () => {
        ...
        let result: string = `filter inline-flex min-w-[40px] max-w-[40px] ${isError ? 'bdr-par-600 color-red' : ''} text-[13px] ${showFocus ? 'bdrf-pacb-400 bdrh-pacb-600' : ''} ${disabled ? 'disabled-light' : ''}`;
        ...
        <>
            <div className={cc()}>
                ...
                <Link to="" onClick={(e) => toggleMenu(e)} className="w-full flex items-center justify-center">
                    <Icon type="feather" name="more-horizontal" size={16} className="w-[16px] h-[16px] pag-600 relative top-[0px]" />
                </Link>
